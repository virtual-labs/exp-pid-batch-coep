...
//	var paper = Raphael("diagram", 1500, 700);
	...
	var tankCcolor = "rgb(86, 176, 84)"; //green
	 ...
	 var time = 1000; // Animation duration for each step
		...
		function tank(x,y,tankColor){
		 paper.path("M" + (x ) + " " + (y) + 
			    " l 100 0 l 0 -100 l -15 -15 l 0 -10 l -70 0 l 0 10 l -15 15 l 0 100").attr({"fill":tankColor});
	 ...
	 .attr({ font: "12px Arial", stroke: "#204687"});  
	    ...
	    var sol1 = paper.text((x+130), (y+100), "Sol 1")
	 .attr({ font: "12px Arial", stroke: "#204687"});  
	   
	    var sol2 = paper.text((x+800), (y+100), "Sol 2")
	 .attr({ font: "12px Arial", stroke: "#204687"});
	 
	     var sol3 = paper.text((x+1020), (y+100), "Sol 3")
	 .attr({ font: "12px Arial", stroke: "#204687"});  
...
//	     feedingTanks();
	  ...
	  var randomAPercentage = Math.floor(Math.random() * (100 - 50 + 1)) + 50;
	  var tankATextReleasePer = paper.text((x+10),(y+20)," Tank A : "+randomAPercentage+" %").attr({
	    font: "20px Arial", fill: tankAcolor,"font-family":"digital-clock-font","font-weight": "bold",opacity: 0}).toFront();
	   
	  var randomBPercentage = Math.floor(Math.random() * (100 - 50 + 1)) + 50;  
	  var tankBTextReleasePer = paper.text((x+10),(y+45)," Tank B : "+randomBPercentage+" %").attr({
	    font: "20px Arial", fill: tankCcolor,"font-family":"digital-clock-font","font-weight": "bold",opacity: 0}).toFront(); 
	  
	  var randomCPercentage = Math.floor(Math.random() * (100 - 40 + 1)) + 40;  
	  var tankCTextReleasePer = paper.text((x+10),(y+70)," Tank C : "+randomCPercentage+" %").attr({
...
//		          jacketHeater(time*20);
		          ...
		          steamFlow.animate({ width: 170 }, (time),function(){

			      steamFlow1.animate({ height: 120, y: y + 342 }, (time),function(){
//				cv.toFront();
				steamFlow1.toFront();
				steamFlow2.animate({ width: 54 }, (time),function(){
					 setTimeout(() => { 
						condensetFlow();
					},time*2);
...
//		var waterColor = "90-#29b6f6-#0288d1-#b3e5fc";
	    ...
	    stroke: "none" // No border
			...
			water.animate({ width: 173 ,x: x + 537}, (time/1.5),function(){
//				jacketHeaterSlowCool(time*3);
				 water1.animate({ height: 122, y: y + 338 }, (time/1.5),function(){
					
					water2.animate({ width: 53 ,x: x + 490}, (time/1.5))
//					,function(){
//			      coldWaterFlowRemove();
//			})
		})
...
//		var waterColor = "90-#29b6f6-#0288d1-#b3e5fc";
	    ...
	    stroke: "none" // No border
			...
			water.animate({ width: 173 ,x: x + 537}, (time/1.5),function(){
				 water1.animate({ height: 123, y: y + 337 }, (time/1.5),function(){
					water2.animate({ width: 53 ,x: x + 490}, (time/1.5))
...
//		var condensateColor = "90-#4590d9-#f0b181-#e69455";
		...
		condensateRect.animate({ width: 223}, (time*2),function()
		{
			condensateRect1.animate({ height: 22}, (time*2))
		})
	...
	//condensetFlowRemove();
	 function condensetFlowRemove(){
		...
		condensateRect.animate({ width: 222}, (time),function()
		{
			condensateRect1.animate({ height: 21}, (time))
		})
...
//	     console.log("time"+time);
		   var Tank_arrAE = [];
		    ...
		    tank_px2 = randomBPercentage/1.5;
...
//					    feedingTanks();
	...
	let dataInterval; // Define the interval globally to reference in other functions
	var dataArr = [];
...
//	     console.log("time"+time);
		   ...
		   var Tank_arrAE = [];
		    ...
		    tank_px1 = randomCPercentage/1.5;
...
//		})
	  ...
	  function tankAEmptyPer(){
		var finalARemain = 100 - randomAPercentage;
	...
	function resumeGif() {
	 ...
	 var tempText = paper.text((x+545),(y+233.5)," \u00B0C").attr({font: "15px Arial", fill :"#fc2d2d","font-weight": "bold"})
	  ...
	  //cv1 textBox
	   ...
	   var cv1RecttUnit = paper.text((x+176	),(y+423)," % ").attr({
	    font: "15px Arial", stroke :"#fc2d2d","font-weight": "bold"})
...
//	     Percentages for each tank
	...
	// Total weight
	var totalWeight = tankAPercentage + tankBPercentage + tankCPercentage;
	...
	var totWt = paper.text((x + 323), (y+165), "0").attr({ 'font-size': 18, "font-family":"digital-clock-font","fill":"#07f74f","font-weight":"bold" });
	 ...
	 var totWtUnit = paper.text((x+364	),(y+166),"ltr").attr({
	    font: "15px Arial", fill :"#fc2d2d","font-weight": "bold"})

	function totalWeightInterval() {
	    let currentPercent = 1; // Start from 1
	    let weightLit = (totalWeight/1.5)
	    weightLit = parseInt(weightLit);
	...
	// Convert to RGB string
	 ...
	 var r1 = paper.rect(x-70,y+100,50,30).attr({"fill":mixedColorRGB});
